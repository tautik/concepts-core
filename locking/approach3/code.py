...
def book(user: User) -> Tuple[Optional[Seat], Optional[str]]:
    """Book a seat for the user using SELECT FOR UPDATE SKIP LOCKED"""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Select the first available seat, skip locked rows
        cursor.execute("""
...
def main():
    ...
    logging.info("Starting seat booking simulation with SKIP LOCKED")
