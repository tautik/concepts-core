...
def book(user: User) -> Tuple[Optional[Seat], Optional[str]]:
    """Book a seat for the user using SELECT FOR UPDATE"""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Select the first available seat and exclusively lock it
        cursor.execute("""
...
def main():
    ...
    logging.info("Starting seat booking simulation with SELECT FOR UPDATE")
    ...
    logging.info(f"Execution took {execution_time:.2f}ms")
